...
def make_band5_unavailable_while_band2_active(
    subarray_node, simulator_factory
):
    (
        _,
        _,
    ...
    capability_argin = json.dumps(
        {
            "B1": CapabilityStates.OPERATE_FULL,  # Band 1 is active
    ...
    logger.info(
        "Band 5 set to UNAVAILABLE while Band 1 is OPERATE_FULL on %s",
    ...
    ), f"Dish {dish_master_sim_1.dev_name} did not become DEGRADED in time"


@then("subarray health state becomes DEGRADED due to unavailable Band 5")
def validate_degraded_subarray_health(subarray_node, event_recorder):
    ...
    assert event_recorder.has_change_event_occurred(
        ...
        "healthState",
        HealthState.DEGRADED,
    ), "Subarray healthState did not transition to DEGRADED"
    ...
    for dish, entries in health_info.items():
        for entry in entries:
            ...
            if "B5" in reason and "UNAVAILABLE" in reason:
                affected_dishes.append((dish, reason))

    assert (
        affected_dishes
    ), "No dish reports Band 5 UNAVAILABLE reason in Subarray healthInfo"

    for dish, reason in affected_dishes:
        logger.info(
            "Dish %s DEGRADED due to Band 5 unavailability | reason=%s",
            dish,
            reason,
        )
