...
@pytest.mark.tmc_csp
@scenario(
    "../features/subarray_001_state_transitions.feature",
...
def subarray_in_obs_fault_state(
    context_fixt,
    subarray_id: str,
    subarray_node_facade: TMCSubarrayNodeFacade,
):
...
@when(
    parsers.parse(
        "the AssignResources command is sent to the subarray {subarray_id}"
    )
)
def send_assign_resources_command(
    ...
    subarray_node_facade.assign_resources(
        command_input_json, wait_termination_condition=False
    )
...
@when(
    parsers.parse(
        "the Configure command is sent to the subarray {subarray_id}"
    )
)
...
@when(
    parsers.parse(
        "the ReleaseResources command is sent to the subarray {subarray_id}"
    )
)
...
@when(
    parsers.parse("the EndScan command is sent to the subarray {subarray_id}")
)
...
@when(
    parsers.parse("the Restart command is sent to the subarray {subarray_id}")
)
...
@when(
    parsers.parse(
        "the subarray {subarray_id} is in the EMPTY state and an observation fault occurs"
    )
)
def subarray_in_empty_state_and_obs_fault(
    context_fixt,
    subarray_id: str,
    subarray_node_facade: TMCSubarrayNodeFacade,
):
...
@when(
    parsers.parse(
        "the subarray {subarray_id} is in the RESOURCING state and the Assigned event is induced"
    )
)
def subarray_in_resourcing_state_and_assigned_event(
    context_fixt,
    subarray_id: str,
    subarray_node_facade: TMCSubarrayNodeFacade,
    csp: CSPFacade,
...
@when(
    parsers.parse(
        "the subarray {subarray_id} is in the RESOURCING state and the Released event is induced"
    )
)
def subarray_in_resourcing_state_and_released_event(
    context_fixt,
    subarray_id: str,
    subarray_node_facade: TMCSubarrayNodeFacade,
    csp: CSPFacade,
...
@when(
    parsers.parse(
        "the subarray {subarray_id} is in the RESOURCING state and the All released event is induced"
    )
)
def subarray_in_resourcing_state_and_all_released_event(
    context_fixt,
    subarray_id: str,
    subarray_node_facade: TMCSubarrayNodeFacade,
    csp: CSPFacade,
...
@when(
    parsers.parse(
        "the subarray {subarray_id} is in the RESOURCING state and an observation fault occurs"
    )
)
def subarray_in_resourcing_state_and_obs_fault(
    context_fixt,
    subarray_id: str,
    subarray_node_facade: TMCSubarrayNodeFacade,
):
...
@when(
    parsers.parse(
        "the subarray {subarray_id} is in the IDLE state and an observation fault occurs"
    )
)
def subarray_in_idle_state_and_obs_fault(
    context_fixt,
    subarray_id: str,
    subarray_node_facade: TMCSubarrayNodeFacade,
):
...
@then(
    parsers.parse(
        "the subarray {subarray_id} should transition to the EMPTY state"
    )
)
def verify_empty_state(
    ...
    ).within_timeout(ASSERTIONS_TIMEOUT).have_events_occurred(
        # let's check for a tuple of events, all of them within the same timeout
...
@then(
    parsers.parse(
        "the subarray {subarray_id} should transition to the RESOURCING state"
    )
)
...
@then(
    parsers.parse(
        "the subarray {subarray_id} should transition to the ABORTING state"
    )
)
...
@then(
    parsers.parse(
        "the subarray {subarray_id} should transition to the CONFIGURING state"
    )
)
...
@then(
    parsers.parse(
        "the subarray {subarray_id} should transition to the IDLE state"
    )
)
...
@then(
    parsers.parse(
        "the subarray {subarray_id} should transition to the READY state"
    )
)
