...
Scenario: Abort configuring DISH.LMC using TMC
    ...
    And the TMC subarray <subarray_id> is busy configuring and DishMaster <dish_ids> is in dishMode CONFIG
    When I issue the Abort command to the TMC subarray 
    Then the DishMaster {dish_ids} reports that command is aborted on lrcr 
