...
def check_telescope_state_is_on(
    central_node_facade: TMCCentralNodeFacade, event_recorder
):
...
def check_csp_is_off(
    central_node_facade: TMCCentralNodeFacade, csp: CSPFacade, event_recorder
):
...
def check_csp_master_is_moved_to_standby(csp: CSPFacade, event_recorder):
    """A method to check CSP controllers State"""
    event_recorder.subscribe_event(csp.csp_master, "State")
    assert event_recorder.has_change_event_occurred(
        csp.csp_master, "State", DevState.STANDBY, lookahead=15
    )


@then("the csp subarray must go to off state")
def check_csp_subarray_is_moved_to_off(csp: CSPFacade, event_recorder):
