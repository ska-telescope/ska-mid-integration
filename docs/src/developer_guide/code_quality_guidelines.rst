...
TMC Mid uses the ``black`` code formatter to format its code. Formatting can be checked using the command ``make python-format``.
...
TMC Mid uses pytest to test its code, with the pytest-cov plugin for
