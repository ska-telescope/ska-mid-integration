...
def create_merged_graph():
    ...
    obs_states = ['EMPTY', 'RESOURCING', 'IDLE', 'CONFIGURING', 'READY',
                  'SCANNING', 'ABORTING', 'ABORTED', 'RESTARTING', 'OBS_FAULT']
    ...
    obs_transitions = [
        ('EMPTY', 'RESOURCING', {'label': 'AssignResources', 'type': 'command'}),
        ('EMPTY', 'OFF', {'label': 'Off', 'type': 'command'}),
        ('EMPTY', 'OBS_FAULT', {'label': 'Observation Fault', 'type': 'event'}),
        ...
        ('RESOURCING', 'OBS_FAULT', {'label': 'Observation fault', 'type': 'event'}),
        ('RESOURCING', 'OP_FAULT', {'label': 'Fatal error', 'type': 'event'}),
        ('IDLE', 'CONFIGURING', {'label': 'Configure', 'type': 'command'}),
        ('IDLE', 'RESOURCING', {'label': 'ReleaseResources', 'type': 'command'}),
        ('IDLE', 'RESOURCING', {'label': 'AssignResources', 'type': 'command'}),
        ...
        ('CONFIGURING', 'OBS_FAULT', {'label': 'Observation Fault', 'type': 'event'}),
        ...
        ('READY', 'OBS_FAULT', {'label': 'Observation Fault', 'type': 'event'}),
...
def draw_curved_edge(ax, posA, posB, label, edge_type, connectionstyle="arc3,rad=0.2"):
    if edge_type == 'command':
        color = 'blue'
    else:
        color = 'red'
    ...
    ax.text(label_pos[0], label_pos[1], label, fontsize=fontsize, ha='center', va='center', color=color,
...
# Draw nodes
op_nodes = [node for node, data in merged_graph.nodes(data=True) if data.get('type') == 'operational']
obs_nodes = [node for node, data in merged_graph.nodes(data=True) if data.get('type') == 'observational']

nx.draw_networkx_nodes(merged_graph, pos, nodelist=op_nodes, node_color='lightblue', node_size=3000, ax=ax)
nx.draw_networkx_nodes(merged_graph, pos, nodelist=obs_nodes, node_color='lightgreen', node_size=3000, ax=ax)
...
for edge in merged_graph.edges(data=True):
    ...
    # Unidirectional edge
    draw_curved_edge(ax, pos[n1], pos[n2], label, edge_type, connectionstyle="arc3,rad=0.3")

# Draw labels
nx.draw_networkx_labels(merged_graph, pos, font_size=15, font_weight='bold', ax=ax)
...
class StateGraphAnalyzer:
    ...
    def print_transitions(self):
        ...
        """
        for i, (start_state, end_state, data) in enumerate(self.graph.edges(data=True), 1):
            trigger = data['label']
            label = compute_edge_label(data['label'], data['type'])
            print(f"{i}. {start_state} --> ({label}) --> {end_state}")

    def print_consecutive_transitions(self):
        """
        Prints all pairs of consecutive transitions in the format:
        start-state --> (trigger1) --> intermediate-state --> (trigger2) --> end-state
        """
        i = 1
        for start_state in self.graph.nodes():
            for mid_state, data1 in self.graph[start_state].items():
                trigger1 = compute_edge_label(data['label'], data['type'])
                for end_state, data2 in self.graph[mid_state].items():
                    trigger2 = compute_edge_label(data['label'], data['type'])
                    print(f"{i}.: {start_state} --> ({trigger1}) --> {mid_state} --> ({trigger2}) --> {end_state}")
                    i += 1
    ...
    def get_outgoing_transitions(self, state):
        ...
        """
        return [(end_state, data['label'], data['type'])
    ...
    def get_incoming_transitions(self, state):
        ...
        """
        return [(start_state, data['label'], data['type'])
    ...
    def get_path_between_states(self, start_state, end_state):
        ...
        try:
            ...
            return [(path[i], path[i+1], self.graph[path[i]][path[i+1]]['label'],
...
analyzer.print_consecutive_transitions()
#
