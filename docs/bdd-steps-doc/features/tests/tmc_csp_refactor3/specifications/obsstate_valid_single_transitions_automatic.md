...
## Feature: subarray 1 State Transitions - Event Triggered


  # ATTENTION

  # these scenarios are obsolete
    # they are not needed anymore
  # use obsstate_valid_single_transitions_commands.feature
...
##  When the subarray 1 is in the RESOURCING state and the Assigned event is induced
##  Then the subarray 1 should transition to the IDLE state
##
##Scenario: RESOURCING to IDLE - AUTO Released (11)
##  When the subarray 1 is in the RESOURCING state and the Released event is induced
##  Then the subarray 1 should transition to the IDLE state
##
##Scenario: RESOURCING to EMPTY - AUTO All released (13)
##  When the subarray 1 is in the RESOURCING state and the All released event is induced
...
##  When the subarray 1 is in the CONFIGURING state and the Ready event is induced
...
##  When the subarray 1 is in the SCANNING state and the ScanComplete event is induced
##  Then the subarray 1 should transition to the READY state
#
...
#  When the subarray 1 is in the ABORTING state and the Abort complete event is induced
...
#  When the subarray 1 is in the RESTARTING state and the Restart Complete event is induced
#  Then the subarray 1 should transition to the EMPTY state
#
...
#  When the subarray 1 is in the OBS_FAULT state and an observation fault occurs
#  Then the subarray 1 should transition to the OBS_FAULT state
