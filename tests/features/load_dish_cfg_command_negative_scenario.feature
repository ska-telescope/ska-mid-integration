Feature: TMC is able to handle the failure when load dish cfg command fails
    ...
    Scenario Outline: TMC returns error message when non existent file is provided in configuration  
        ...
        | invalid_file_name                 | Error in Loading Dish VCC map json file gitlab://gitlab.com/ska-telescope/ska-tmc/ska-tmc-simulators?t1#tmdata not found in SKA CAR - make sure to add tmdata CI! |
